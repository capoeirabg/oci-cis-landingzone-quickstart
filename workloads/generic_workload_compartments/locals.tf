...
locals {
  ...
  # Tags
  landing_zone_tags = { "cis-landing-zone" : fileexists("${path.module}/../release.txt") ? "${var.service_label}-${var.branding_tag}/${file("${path.module}/../release.txt")}" : "${var.service_label}-${var.branding_tag}" }
}